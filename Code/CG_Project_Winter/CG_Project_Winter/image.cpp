...
void Image::SaveTGA(const char *filename) const
{
    ...
    // The data
    // Flip y so that (0,0) is bottom left point.
    for (int y = height - 1; y >= 0; y--) {
        for (int x = 0; x < width; x++) {
            glm::dvec3 v = GetPixel(x, y);

            // Gamma.
            double inv_gamma = 0.5;
            v[0]             = pow(v[0], inv_gamma);
            v[1]             = pow(v[1], inv_gamma);
            v[2]             = pow(v[2], inv_gamma);
...
Image *Image::LoadTGA(const char *filename)
{
    ...
    for (int y = height - 1; y >= 0; y--) {
        for (int x = 0; x < width; x++) {
            ...
            g = ReadByte(file);
            r = ReadByte(file);
            glm::dvec3 color(r / 255.0, g / 255.0, b / 255.0);
...
Image *Image::Compare(Image *img1, Image *img2)
{
    ...
    for (int x = 0; x < img1->Width(); x++) {
        for (int y = 0; y < img1->Height(); y++) {
            glm::dvec3 color1 = img1->GetPixel(x, y);
            glm::dvec3 color2 = img2->GetPixel(x, y);
            glm::dvec3 color3 = glm::dvec3(fabs(color1.r - color2.r),
